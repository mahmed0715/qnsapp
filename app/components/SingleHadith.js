...
const Single = ({item})=> {
    ...
    return (
      ...
      <View style={{flexDirection:'row'}}>
        <Text style={[theme.textColor, {alignSelf:'flex-start', textAlign:'left', paddingLeft:5, paddingRight:3}]}>{item.id}</Text>
        <View style={{paddingRight:10, marginRight: 10, paddingLeft:5}}> 
          {<Text style={[theme.textColor]}> {item.hadith_narrated}</Text> }
          <Text style={[theme.textColor]}> {item.text_details}</Text>
        </View>
      </View>
